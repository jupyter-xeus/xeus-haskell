...
void interpreter::execute_request_impl(send_reply_callback cb,
  ...
  if (!config.silent) {
    ...
    if (!output.empty()) {
      nl::json pub_data;
      const bool rich_display =
          exec_result.mime_type == "text/html" ||
          exec_result.mime_type == "text/latex" ||
          exec_result.mime_type == "text/markdown";
      pub_data[rich_display ? exec_result.mime_type : "text/plain"] = output;
      if (rich_display) {
        display_data(std::move(pub_data), nl::json::object(),
                     nl::json::object());
      } else {
...
nl::json interpreter::kernel_info_request_impl() {
  ...
  const std::string implementation = "xhaskell-mhs";
  ...
  const std::string banner = "xhaskell-mhs";
